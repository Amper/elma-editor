...
import {
  SPRING_TENSION_COEFFICIENT, SPRING_RESISTANCE_COEFFICIENT,
  LEFT_WHEEL_DX, LEFT_WHEEL_DY, RIGHT_WHEEL_DX, RIGHT_WHEEL_DY,
  VOLT_DELAY, HEAD_RADIUS, VOLT_ANGULAR_IMPULSE, VOLT_RESIDUAL_CHANGE,
  GAS_TORQUE, BRAKE_FORCE, BRAKE_FRICTION, MAX_SPIN_OMEGA, GRAVITY,
} from '../core/Constants';
import type { MotorState } from './MotorState';
import { MotorGravity } from './MotorState';
import type { Rigidbody } from './Rigidbody';
import { rigidbodyMovement, bodyMovement } from './PhysicsMove';
import { getTwoAnchorPoints, getTouchingObject } from './PhysicsCollision';
import type { Segments } from './Segments';
import { WavEvent, type EventBuffer } from '../game/EventBuffer';
...
function calculateForces(
  ...
  const mountingPointVelocity = rodPerp.scale(mot.bike.angularVelocity).add(mot.bike.v);
...
function calculateFriction(mot: MotorState, stretchV: Vec2, velocityV: Vec2): void {
...
export function step(
  ...
  if (brake) {
    let dalfa = mot.leftWheel.rotation - (mot.bike.rotation + mot.leftWheelBrakeRotation);
    let domega = mot.leftWheel.angularVelocity - mot.bike.angularVelocity;
    leftWheelTorque = -BRAKE_FORCE * dalfa - BRAKE_FRICTION * domega;

    dalfa = mot.rightWheel.rotation - (mot.bike.rotation + mot.rightWheelBrakeRotation);
  ...
  const leftResult = calculateForces(
    mot, mot.leftWheel, i1, j1, LEFT_WHEEL_DX, LEFT_WHEEL_DY, leftWheelTorque
  );
  const rightResult = calculateForces(
    mot, mot.rightWheel, i1, j1, RIGHT_WHEEL_DX, RIGHT_WHEEL_DY, rightWheelTorque
  );
  ...
  switch (mot.gravityDirection) {
    case MotorGravity.Down:
      ...
      rigidbodyMovement(mot.bike, bikeForce.sub(Vec2j.scale(mot.bike.mass * GRAVITY)),
                         bikeTorque, dt, false, segments, eventBuffer);
      rigidbodyMovement(mot.leftWheel, leftResult.wheelForce.sub(Vec2j.scale(mot.leftWheel.mass * GRAVITY)),
                         leftWheelTorque, dt, true, segments, eventBuffer);
      rigidbodyMovement(mot.rightWheel, rightResult.wheelForce.sub(Vec2j.scale(mot.rightWheel.mass * GRAVITY)),
                         rightWheelTorque, dt, true, segments, eventBuffer);
      break;
    case MotorGravity.Up:
      gravityVec = new Vec2(0, 1);
      bodyMovement(mot, gravityVec, i1, j1, dt);
      rigidbodyMovement(mot.bike, bikeForce.add(Vec2j.scale(mot.bike.mass * GRAVITY)),
                         bikeTorque, dt, false, segments, eventBuffer);
      rigidbodyMovement(mot.leftWheel, leftResult.wheelForce.add(Vec2j.scale(mot.leftWheel.mass * GRAVITY)),
                         leftWheelTorque, dt, true, segments, eventBuffer);
      rigidbodyMovement(mot.rightWheel, rightResult.wheelForce.add(Vec2j.scale(mot.rightWheel.mass * GRAVITY)),
                         rightWheelTorque, dt, true, segments, eventBuffer);
      break;
    case MotorGravity.Left:
      gravityVec = new Vec2(-1, 0);
      bodyMovement(mot, gravityVec, i1, j1, dt);
      rigidbodyMovement(mot.bike, bikeForce.sub(Vec2i.scale(mot.bike.mass * GRAVITY)),
                         bikeTorque, dt, false, segments, eventBuffer);
      rigidbodyMovement(mot.leftWheel, leftResult.wheelForce.sub(Vec2i.scale(mot.leftWheel.mass * GRAVITY)),
                         leftWheelTorque, dt, true, segments, eventBuffer);
      rigidbodyMovement(mot.rightWheel, rightResult.wheelForce.sub(Vec2i.scale(mot.rightWheel.mass * GRAVITY)),
                         rightWheelTorque, dt, true, segments, eventBuffer);
      break;
    case MotorGravity.Right:
      gravityVec = new Vec2(1, 0);
      bodyMovement(mot, gravityVec, i1, j1, dt);
      rigidbodyMovement(mot.bike, bikeForce.add(Vec2i.scale(mot.bike.mass * GRAVITY)),
                         bikeTorque, dt, false, segments, eventBuffer);
      rigidbodyMovement(mot.leftWheel, leftResult.wheelForce.add(Vec2i.scale(mot.leftWheel.mass * GRAVITY)),
                         leftWheelTorque, dt, true, segments, eventBuffer);
      rigidbodyMovement(mot.rightWheel, rightResult.wheelForce.add(Vec2i.scale(mot.rightWheel.mass * GRAVITY)),
                         rightWheelTorque, dt, true, segments, eventBuffer);
      break;
  }
...
export function checkCollisions(
  ...
  while (hadFood) {
    hadFood = false;
    const indices = [
      getTouchingObject(objects, mot.leftWheel.r, mot.leftWheel.radius, isFlagTag),
      getTouchingObject(objects, mot.rightWheel.r, mot.rightWheel.radius, isFlagTag),
...
export function calculateFrictionValue(): number {
  return maxFriction;
}