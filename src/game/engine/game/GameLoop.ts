...
import {
  PHYSICS_DT, TIME_TO_CENTISECONDS, VOLT_DELAY,
} from '../core/Constants';
import { MotorGravity, createMotorState, initMotor, type MotorState } from '../physics/MotorState';
import { Segments } from '../physics/Segments';
import { step, resetStepper, calculateHeadPosition, checkCollisions, calculateFrictionValue } from '../physics/Stepper';
import { EventBuffer, WavEvent } from './EventBuffer';
import { Recorder, type BikeSound } from './Recorder';
import { InputManager, type KeyBindings, DEFAULT_KEYS } from '../input/InputManager';
...
export type GameResult = 'playing' | 'dead' | 'won' | 'escaped';
...
export function createGame(
  ...
  const state: GameState = {
    ...
    sound: { motorFrequency: 1.0, gas: false, frictionVolume: 0 },
    lastVoltTime: -100,
    lastTurnTime: -1000,
    flipFactor: 1.0,
...
function physicsStep(state: GameState, dt: number): void {
  ...
  // Step physics
  step(
    motor, state.gameTime, dt,
    input.isDown(keys.gas),
    input.isDown(keys.brake),
    rightVolt, leftVolt,
    segments, eventBuffer
  );

  // Check death/objects
  const result = checkCollisions(motor, state.level.objects, segments, eventBuffer, false);
  if (result === 0) {
    // Dead
    state.sound.frictionVolume = 0;
    state.sound.motorFrequency = -1;
    state.sound.gas = false;
    state.recorder.storeFrames(motor, state.gameTime, state.sound);
    state.result = 'dead';
    return;
  }

  // Update sound state
  state.sound.frictionVolume = calculateFrictionValue();
  if (motor.flippedBike) {
    state.sound.motorFrequency = Math.abs(motor.leftWheel.angularVelocity) * 0.025;
  } else {
    state.sound.motorFrequency = Math.abs(motor.rightWheel.angularVelocity) * 0.025;
  }
  if (state.sound.motorFrequency > 30.0) state.sound.motorFrequency = 30.0;
  state.sound.motorFrequency = 2.0 - Math.exp(-state.sound.motorFrequency);
  state.sound.gas = input.isDown(keys.gas);
  ...
  while ((evt = eventBuffer.get()) !== null) {
    ...
    state.recorder.storeEvent(state.gameTime, evt.eventId, evt.volume, evt.objectId);
  }
}
