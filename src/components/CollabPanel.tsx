...
export function CollabPanel({ hidden = false }: { hidden?: boolean }) {
  ...
  // Allow joining via prop (auto-join from URL param)
  const autoJoinRoom = useEditorStore((s) => (s as any).__autoJoinRoom) as string | undefined;
  useEffect(() => {
    if (autoJoinRoom && mode === 'idle') {
      ...
      const timer = setTimeout(() => {
        ...
        clientRef.current = client;
        setCollabClient(client);
        client.connect(autoJoinRoom, name);
        setRoomId(autoJoinRoom);
        setMode('joined');
        // Clean up the auto-join marker
        useEditorStore.setState({ __autoJoinRoom: undefined } as any);
      }, 100);
      return () => clearTimeout(timer);
    }
  }, [autoJoinRoom, mode, userName, setCollabClient]);
