...
export function computeInverse(level: Level, op: Operation): Operation {
  switch (op.type) {
    ...
    case 'setPolygonsGrass': {
      // If all had the same grass value, return the opposite batch.
      // Otherwise, need individual operations in a batch.
      const ops: Operation[] = [];
    ...
    case 'removeVertex': {
      ...
      if (!poly) return { type: 'batch', operations: [] }; // no-op
    ...
    case 'removeVertices': {
      // Each entry needs to restore the removed vertices
      const ops: Operation[] = [];
      for (const entry of op.verts) {
        const poly = level.polygons.find((p) => p.id === entry.polyId);
        if (!poly) continue;
        // Insert in ascending order to restore correctly
    ...
    case 'updateObjects': {
      // Capture old values for each object
      const ops: Operation[] = [];
      for (const id of op.ids) {
        const obj = level.objects.find((o) => o.id === id);
        if (!obj) continue;
    ...
    case 'replacePolygons': {
      // Inverse: remove the added, restore the removed
    ...
    case 'batch': {
      // Inverse of a batch is the batch of inverses in reverse order
      // Each inverse is computed against the level state before that sub-op
      // For simplicity, we compute all inverses against the original level
      // This is correct for independent operations within a batch
